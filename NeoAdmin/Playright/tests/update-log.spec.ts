...
test.describe('NeoUpdateLog 更新日志', () => {
  test('演示页从 JSON 文件展示更新日志时间线', async ({ page }) => {
    ...
    const changelog = JSON.parse(fs.readFileSync(changelogPath, 'utf8')) as { subject: string }[];
    const firstSubject = changelog[0]?.subject;
    ...
    await expect(page.getByText(firstSubject!, { exact: true })).toBeVisible({ timeout: stepTimeout });
  });
});
